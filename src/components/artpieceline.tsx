...
export const ArtPieceLine = (props: ArtPieceLineProps) => {
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    drop: (item: { id: string }, monitor) => {
      emitSwap({ source: item.id as string, destination: props.artPiece._id });
      emitItemOver("");
    },
    collect: (monitor) => ({
  ...
  const Dotted = (props: { text: string; chars: number }) => {
    ...
    if (props.text && props.text.length >= props.chars) {
      return (
        <p onClick={(e) => show(e)} className="smallerText pointer">
  ...
  return (
    <>
      <div
        ...
        onClick={(e) => selectRow(props.artPiece._id)}
        className={`gridTable1 paddingAllSmall gap24 borderBottom ${itemOver ? "over" : ""}`}
      >
        <div ref={drag as unknown as Ref<HTMLDivElement>} className="draggable">
          <img className="smallImageW" src={handle} />
        </div>
        <img
          className="iconImage round8 zoom"
          src={`data:image/png;base64,${props.artPiece.b64Image}`}
          onClick={(e) => {
            e.stopPropagation();
            loadOtherImages(props.artPiece.otherMedia!.map((item) => item.url));
            loadImage(props.artPiece?.pictures[0]?.url);
            setShowImage(props.artPiece.title);
          }}
        />
        <div className="artinfo">
          <div className="smallText">
            <span
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/edit/${props.artPiece._id}`);
              }}
            >
          ...
          <div className="artmeasure smallerText">
            ...
            <div>
              {props.artPiece.creation_date
                ? props.artPiece.creation_date.toString().substring(0, 4)
                : ""}
            </div>
            <div className="boldText">
              {new Intl.NumberFormat("de-de").format(props.artPiece.price)}
            </div>
          </div>
          <div className="smallerText">
            {displayLabel(props, "arttype", t("general.notset"))}
          </div>
          <div className="smallerText">
            {displayLabel(props, "status", t("general.notset"))}
          </div>
        </div>
        <ThreeDots>
          <ul>
            <li onClick={() => router.push(`/edit/${props.artPiece._id}`)}>
              {t("general.edit")}
            </li>
            <li onClick={() => setShowDeleteForm(props.artPiece._id)}>
              {t("general.delete")}
            </li>
            <li onClick={() => emitAddMoreMedia({ id: props.artPiece._id })}>
              {t("general.addmoremedia")}
            </li>
          </ul>
        </ThreeDots>
      </div>
      ...
      >
        ...
        <div className="flex">
          <img
            ...
            src={`data:image/png;base64,${props.artPiece.b64Image}`}
          />
          <p className="strong center paddingV paddingH">
            {props.artPiece.title}
          </p>
        </div>
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    drop: (item: { id: string }, monitor) => {
      emitSwapOrder({
        ...
        destination: props.artPiece._id,
      });
      emitItemOver("");
    },
    collect: (monitor) => ({
      ...
      >
        ...
        <div className="buttonblock">
          ...
          <button
            ...
            onClick={() => {
              setShowDeleteForm("");
              props.removePieceFromList(showDeleteForm);
            }}
          >
            {t("general.delete")}
          </button>
