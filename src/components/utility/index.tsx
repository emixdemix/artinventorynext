...
import { ArtPiece, ArtSelection, Category, Customer, GeneralAPIError, KeyValue, PictureDao, Reports, Shows, StoreContext, Timeout } from '../../interfaces'
...
export const uploadArtPiece = async (formData: FormData) => {
   ...
   try {
      const response = await axios.post(`/api/artpiece`, formData, {
         headers: { 'X-Token': localSession, 'Content-Type': 'multipart/form-data' }
      })
      return true
   } catch (e) {
      return false
...
export const apiMoveMedia = async (payload: KeyValue) => {
   const localSession = localStorage.getItem('session')
   try {
      const response = await axios.post(`/api/movemedia`, {
         ...payload
      }, {
...
export const updateSelectionOrder = async (data: {selections:string[], selectionId:string}) => {
   const localSession = localStorage.getItem('session')
   try {
...
export const doLogin = async (values: KeyValue) => {
   try {
      const response = await axios.post(`/papi/login`, {
...
export const doSendLink = async (values: KeyValue) => {
   try {
      const response = await axios.post(`/papi/sendlink`, {
         "g-recaptcha-response": values.token,
         email: values.email,
      }, {
         headers: {
...
export const doChangePassword = async (values: KeyValue):Promise<GeneralAPIError> => {
   try {
      const response = await axios.post(`/papi/changepassword`, {
...
export const doRegister = async (values: KeyValue) => {
   try {
      const response = await axios.post(`/papi/register`, {
         "g-recaptcha-response": values.token,
         username: values.email,
         password: values.password
      }, {
...
export const doReset = async (values: KeyValue) => {
   try {
      const response = await axios.post(`/papi/reset`, {
         "g-recaptcha-response": values.token,
         email: values.email
      }, {
         headers: {
...
export const doConfirm = async (values: KeyValue) => {
   try {
      const response = await axios.post(`/papi/confirm`, {}, {
         headers: {
...
export const doAutoLogin = async (values: KeyValue) => {
   try {
      const response = await axios.post(`/papi/autologin`, {}, {
         headers: {
            'X-Token': values.token
         }
      })
...
export const displayLabel = (props:{artPiece:ArtPiece}, which:string, notset:string) => {
...
export const Dotted = (props: { text: string, chars: number, className?: string}) => {
   ...
   if (props.text && props.text.length >= props.chars) {
      return <p onClick={(e) => show(e)} className={`smallerText pointer ${props.className ? props.className : ""}`}>{showAll ? props.text : `${props.text.substring(0, props.chars)}...`}</p>
   } else {
      return <p className={`smallerText ${props.className ? props.className : ""}`}>{props.text}</p>
   }
}
...
export const apiSaveCustomer = async (data: KeyValue):Promise<GeneralAPIError> => {
   const localSession = localStorage.getItem('session')
...
export const apiGetFeedback = async (data: KeyValue):Promise<GeneralAPIError> => {
   const localSession = localStorage.getItem('session')
...
export const deleteCategory = async (id: string):Promise<AxiosResponse<any, any>| null> => {
   const localSession = localStorage.getItem('session')
...
export const getArtPiece = async (id: string): Promise<ArtPiece | undefined> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
...
export const getSelection = async (id: string): Promise<ArtSelection | undefined> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
...
export const apiDeleteCustomer = async (id: string): Promise<Customer | undefined> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
...
export const apiDeleteSelection = async (id: string): Promise<ArtSelection | undefined> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
...
export const apiRemoveFromSelection = async (data: { ids: string[], selectionId: string }): Promise<ArtSelection | undefined> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
         const response = await axios.patch(`/api/selection`, {
            ids: data.ids,
...
export const apiSaveMediaToArtipiece = async (data:{selections: string[], artPieceId: string}): Promise<boolean> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
         const response = await axios.patch(`/api/addmediatoartpiece`, {
...
export const saveNewList = async (selections: string[], name: string, selectionId: string): Promise<boolean> => {
   const localSession = localStorage.getItem('session')
   if (localSession) {
      try {
         const response = await axios.patch(`/api/selections`, {
            selections,
            name,
            selectionId
         }, {
...
export const updateArtPiecesOrder = async (ids: string[]): Promise<boolean> => {
   ...
   if (localSession) {
      try {
         const response = await axios.post(`/api/orderartpieces`, { ids },
            {
               headers: {
                  'X-Token': localSession
               }
...
export const GetImage = (props: { file: File }) => {
   ...
   return (
      <img id="image" className="" src={url} />
   )
}
...
export const { useSelectRowListener, emitSelectRow } = createEvent("SelectRow")<string>();
export const { useSwapListener, emitSwap } = createEvent("Swap")<{ source: string, destination: string }>();
export const { useItemOverListener, emitItemOver } = createEvent("ItemOver")<string>();
export const { useLoginListener, emitLogin } = createEvent("Login")<KeyValue>();
export const { useWaitingListener, emitWaiting } = createEvent("Waiting")<Timeout>();
export const { useErrorListener, emitError } = createEvent("Error")<string>();
export const { useStoreListener, emitStore } = createEvent("Store")<{ key: string, value: Object, store?: boolean }>();
export const { useAppStateListener, emitAppState } = createEvent("AppState")<StoreContext>();
export const { useSwapOrderListener, emitSwapOrder } = createEvent("SwapOrder")<{ source: string, destination: string }>();
export const { useAddMoreMediaListener, emitAddMoreMedia } = createEvent("AddMoreMedia")<{ id:string }>();
export const { useFileDroppedListener, emitFileDropped } = createEvent("FileDropped")<KeyValue>();
