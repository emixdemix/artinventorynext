...
export const getSalesArtPieces = async (
  query: KeyValue,
): Promise<KeyValue[]> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
...
export const sellArtPieces = async (
  query: KeyValue,
): Promise<UpdateResult<Customer>> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
...
export const addMediaToArtpiece = async (
  query: KeyValue,
): Promise<UpdateResult<ArtPiece>> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
  const collection = db?.collection<ArtPiece>(ARTPIECES_COLLECTION);
...
export const updateSelection = async (
  ...
  if (query.selectionId) {
    let final;
    const selection = await collection.findOne({
      owner: query.owner,
    ...
    if (selection) {
      ...
      const result = await collection.updateOne(
        ...
        {
          $set: {
            artpieces: final.map((item) => new ObjectId(item as string)),
          },
        },
      );
      return {
...
export const removeFromSelection = async (
  query: KeyValue,
): Promise<ErrorCode> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
...
export const getArtSelectionsWithImages = async (
  query: KeyValue,
): Promise<ArtPieceWithImage[]> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
...
export const getArtWithImages = async (
  query: KeyValue,
): Promise<ArtPieceWithImage[]> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
...
export const getCustomers = async (
  ...
  const pipeline = [{ $match: query }, { $sort: { name: 1 } }];
...
export const deleteShow = async (
  ...
  const pieces = await collection.deleteOne({
    owner,
...
export const deleteMedia = async (
  ...
  if (media) {
    ...
    const shows = db.collection<Shows>(SHOWS_COLLECTION);
    await Promise.all([
      artpieces.updateMany({ owner, pictureId: id }, {
        $pull: { pictureId: id },
      } as never),
      artpieces.updateMany({ owner, extraMedia: id }, {
        $pull: { extraMedia: id },
      } as never),
      shows.updateMany({ owner, pictureId: id }, {
        $pull: { pictureId: id },
      } as never),
    ]);
...
export const saveLoginInfo = async (
  user: WithId<User>,
): Promise<InsertOneResult<Logins> | null> => {
  const client = await getDbClient();
  const db = client.db(ARTINVENTORY_DB);
...
export const orderArtPieces = async (params: {
  owner: ObjectId;
  ids: string[];
}) => {
  let order = 0;
  ...
  params.ids.forEach((id) => {
    promises.push(
      new Promise((resolve) => {
        collection
          .updateOne(
            { _id: new ObjectId(id), owner: params.owner },
            { $set: { order } },
          )
          .then((response) => {
            resolve({});
...
export const getStatistics = async (owner: ObjectId) => {
  ...
  const db = client.db(ARTINVENTORY_DB);
  const collection = db?.collection<ArtPiece>(ARTPIECES_COLLECTION);
  let pieces = collection.aggregate<WithId<ArtPiece>>(
    getStatisticsPipeline(owner, "status"),
  );
  const statuses = await pieces.toArray();
  pieces = collection.aggregate<WithId<ArtPiece>>(
    getStatisticsPipeline(owner, "category"),
  );
  const categories = await pieces.toArray();
  pieces = collection.aggregate<WithId<ArtPiece>>(
    getStatisticsPipeline(owner, "arttype"),
  );
  ...
  records.forEach((element) =>
    promises.push(
      new Promise(async (resolve) => {
        ...
        b64 = response;
        const art = await collection.updateOne(
          { _id: pictureId },
          {
            $set: {
              mediaImage: b64,
...
export const movePictureToFolder = async (
  pictureId: string,
  path: string,
  owner: WithId<User>,
) => {
  ...
  if (picture && picture.length > 0) {
    ...
    try {
      await copyObject(
        ARTINVENTORY_BUCKET,
        `${ARTINVENTORY_BUCKET}/${picture[0].owner.toString()}/${picture[0].url}`,
        destination,
      );
      ...
      await deleteObject(
        ARTINVENTORY_BUCKET,
        `${picture[0].owner.toString()}/${picture[0].url}`,
      );
